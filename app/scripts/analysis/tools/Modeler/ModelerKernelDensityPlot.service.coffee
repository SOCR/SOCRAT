...
module.exports = class KernelDensityPlot extends BaseService
  ...
  plotHist: (bins, container, arr, _graph, gdata, x, height, width, data) ->
    console.log("Plotting Kernel Distribution")
    # slider
    $('#slidertext').remove()
    ...
    #can change
    bandwith = 4
    kde = kernelDensityEstimator(epanechnikovKernel(bandwith), x.ticks(100));
  ...
  drawHist: (_graph, data, container, gdata, width, height, ranges, parameters) ->
